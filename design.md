...
| REQ-EXEC-003 *(live-capital)* | `execution_controller.jl` (`reconcile!`), `venues/ibkr.jl` (`positions`), `module_10_feedback` | *(none — step 6)* | runtime (reconcile + halt) | partial — **step 2:** `reconcile!` compares expected vs broker `positions()` and halts on divergence beyond tolerance. Expected positions are optimistic until the ledger is authoritative (step 3); halt is controller-wide until per-pool state (step 4). |
