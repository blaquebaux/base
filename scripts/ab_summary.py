#!/usr/bin/python3
# =============================================================================
# ab_summary.py — READ-ONLY Monday A/B check-in across the spine-family paper books.
#
# Queries each configured Alpaca paper account (single +DBA spine, split-universe
# spine, single+multi-horizon spine), each skipped cleanly until its key file
# exists; reads the day's job logs, and writes a side-by-side summary (each active
# leg vs the SINGLE control) to
...
STRATS = [  # label, env file, expected job logfile prefix
    ...
    # (Blunt and other family sleeves live in their own repos with their own monitoring.)
]
