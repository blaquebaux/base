...
| **blaquebaux-bridgewater** | Bridgewater's All-Weather & Pure Alpha | concept — All-Weather = replicable risk-parity (overlaps the spine; testable via SPY/TLT/GLD/DBC/DBA + RPAR); Pure Alpha = discretionary macro, a documented data gap |
| **blaquebaux-bind** | the rest of the hedge-fund landscape (Millennium, Citadel, pod shops) | concept — the pod shops' returns are private; the only price window is hedge-fund-replication liquid alts (QAI/MNA/BTAL/DBMF). Is the replicable residue anything but watered-down beta? |
| **blaquebaux-blackstone** | listed private-equity managers (BX/KKR/Apollo/Carlyle/Ares) | concept — "buy, prop, exit" model's real exposure; prior = levered financials + credit-cycle amplifier, not diversification. Fully price-testable (the PE managers are listed) |
| **blaquebaux-benefactors** | foreign creditors — US Treasury holdings by country vs their economies/markets | concept — **most data-gapped**: the core TIC foreign-holdings series isn't on any price feed (external data needed); only the creditor-country equity side is proxyable. Macro study, confounded causality |
