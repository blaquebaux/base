...
| **blaquebaux-brigade** | expanded capstone — keepers **+ curated void-fillers** | research validated (**breadth beats purity**) — risk parity over the 6 keepers *plus* the uncorrelated near-keepers that fill genuine voids (crypto/[bitdollar](https://github.com/blaquebaux/bitdollar), defensive/[buffett](https://github.com/blaquebaux/buffett), reversal/[bounce](https://github.com/blaquebaux/bounce)) lifts the book **+1.49→+1.54** (2021–26 window, crypto/BITO history). Lift from the genuinely uncorrelated adds (crypto corr +0.35, reversal +0.14); defensive (+0.93) rides along. Fill the voids that *actually* diversify · **governed live rail built (dry-run PASS)** |
...
| **blaquebaux-bombs-away** | fragility monitor — the tail's smoke detector | research done (**monitor, not a sleeve; honest RED on the composite**) — watches market-observable fragility tells (global yields, credit spreads, breadth/concentration, complacency, havens, credit-sensitive equities). The composite "bomb clock" shows **NO forward-drawdown edge** (flat ~−5% at any level) → it FAILS as a market-timing switch (the bear-null law again); it rose *into* the slow crises (COVID/2022) but missed the sudden 2025 shock. Honest use: a per-tell **dashboard** + a bounded **tail-hedge sizing** nudge — emits `bombs_away.txt`; Booster leans its always-on TAIL sleeve off the **hottest lit fuse** (never a switch to cash). Right now the **rates fuse is lit (global yields climbing, 88th pctile)** while the blend is lulled to 6/100 — the "what, me worry?" calm quantified. Data-honesty: ETF proxies reach only 2019+, and SpaceX/private marks, Nvidia vendor-financing circularity, funding stress & MOVE are **flagged, not faked** (FRED/MOVE/filings upgrade + real-history validation specced in `docs/V2_SPEC.md`). Pairs with the black-swan TAIL sleeve: **bombs-away watches, TAIL insures** |
...
| # | name | branch | notes |
|---|------|--------|-------|
| 1 | ~~**blaquebaux-brace**~~ | volatility / vol-risk-premium | ✅ **built — honest null.** Harvesting the VRP (bleed's counterparty) is the insurer's side (negative skew); via listed ETFs it lags the index gated or not, SVXY is a −95% landmine, and it's not a portable-alpha source. On the shelf |
| 2 | ~~**blaquebaux-bounty**~~ | carry (cross-asset) | ✅ **built — honest null.** ETF-harvestable carry (credit/term/EM spread) is a crash-risk premium, not clean alpha: +0.39 Sharpe, corr-SPY +0.80, skew −2.02, crisis-corr +0.84. FX carry (DBV) delisted; the pure futures carry factor is out of reach. On the shelf |
| 3 | ~~**blaquebaux-bite**~~ | event-driven / merger-arb | ✅ **built — honest null.** Merger arb (MNA) is watered-down (excess-over-cash +0.7%/yr, skew −2.36, crisis-corr +0.70 — deals break when markets fall); spin-offs (CSD) are just small-cap beta. The real edge is unreplicable via ETF. On the shelf |
| 4 | ~~**blaquebaux-bridge**~~ | statistical arbitrage / pairs | ✅ **built — honest null.** The ideal portable-alpha *vehicle* (beta +0.01, crisis-corr +0.09) but no alpha to carry — the classic pairs edge has decayed (book Sharpe +0.15 after cost). Profile is not edge. On the shelf |
