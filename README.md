...
| **blaquebaux-benefactors** | foreign creditors — US Treasury holdings by country vs their economies/markets | research (first-pass partial) — data source resolved: **FRED** (Treasury TIC, `FORLTTREASPOS<code>`), supplied by hand into `research/data/` (this box can't reach FRED). Holding *flow* characterized for the first series; market test pending the **country** behind the code + more history/countries (FRED per-country series start ~2023). Confounded causality; a macro read, not an edge |
